...
const { t } = useI18n()
const { btnStyle, textFieldStyling, selectStyling, textAreaStyle } =
  useAppStyling()
  ...
  (v: string | null | ItemSummarySchema) => {
    ...
    if (typeof v === 'object') {
      return (
        ...
        t('items.item.edit.name_required')
      )
    }
...
const descriptionRules = ref([
  (v: string | null) =>
    (v ?? '').length < 1024 || t('items.item.edit.description_too_long')
])
...
const loadCategories = async () => {
  ...
  const { success, data } = await itemEndpoint.getAllCategories()
  if (!success) {
    close()
    return
  }
          ...
          <v-row>
            ...
            <v-col cols="12">
              <v-textarea
                ...
                :append-inner-icon="
                  item.description !== description ? 'mdi-undo' : ''
                "
              ...
              >
                <template v-slot:selection="{ item, index }">
                  ...
                  <v-chip
                    ...
                    :text="
                      t(
                        'items.item.edit.categories_overflow',
