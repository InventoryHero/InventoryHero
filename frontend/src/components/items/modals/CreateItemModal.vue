...
const nameRules = ref([
  (v: string | null | ItemSummarySchema) => {
    ...
    if (typeof v === 'object') {
      return (
        !!(v as ItemSummarySchema).name?.trim() ||
        t('create.item.form.name_required')
      )
    }
...
const notesRules = ref([
  (v: string | null) =>
    (v ?? '').length < 1024 || t('create.item.form.notes_too_long')
])
const descriptionRules = ref([
  (v: string | null) =>
    (v ?? '').length < 1024 || t('create.item.form.description_too_long')
])
const parentRules = ref([
  (v: StorageResponseSchema | null) =>
    !!v || t('create.item.form.parent_needed')
])
...
const handleAddItemInstance = async () => {
  ...
  loading.value = true
  const { success } = await itemEndpoint.createItemInstance(
    (item.value as ItemSummarySchema).id,
    newItem
  )
  if (!success) {
    loading.value = false
                ...
                >
                  <template v-slot:selection="{ item, index }">
                    ...
                    <v-chip
                      ...
                      :text="
                        t(
                          'create.item.form.categories_overflow',
                          categories.length - 2
                        )
                      "
                    />
