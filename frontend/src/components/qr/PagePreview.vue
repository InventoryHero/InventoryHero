...
import {RoomResponseSchema, BoxResponseSchema, StorageType} from "@/api/types/storage.ts";
...
const {columns, rows, margins, paper, borderThickness} = storeToRefs(printSettings)
...
const labelHeightInMm = computed(() => {
  const height = (paperSizes[paper.value].height-margins.value.top-margins.value.bottom)/rows.value
  console.log(height)
...
const getStorageIndex = (page: number, row: number, col: number) => {
  return (page - 1) * labelsPerPage.value + (row - 1) * (columns.value) + (col-1)
}
...
<template>
  ...
  >
    <v-card-text
        class="text-center flex-1-1 overflow-hidden "
    >
              ...
              >
                ...
                >
                  <v-col
                      ...
                      class="fill-height"
                      :style="{
                        border: (getStorageIndex(page, row, col) < objects.length) ? border : undefined
                      }"
                  >
