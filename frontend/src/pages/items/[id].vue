...
const loadItem = async (
  showLoader: boolean,
  loadRequestingStorage: string | undefined = undefined
) => {
...
<template>
  ...
  <template v-else>
    ...
    >
      ...
      <template
        v-for="(item, index) in breadcrumbs.slice(-2)"
        :key="item.id"
      >
        <v-breadcrumbs-item :to="getStoragePath(item.id, item.type)">
          <span class="d-flex align-center">
            <v-icon :icon="getStorageIcon(item.type)" />
            {{ item.name }}
          </span>
        </v-breadcrumbs-item>
        <v-icon
