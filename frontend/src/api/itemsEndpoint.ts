...
export default (api: AxiosInstance) => {
  const getAllItemsSummary = async (): Promise<
    ApiResponse<Array<ItemSummarySchema>>
  > => {
  ...
  const getAllCategories = async (): Promise<
    ApiResponse<Array<CategoryReadSchema>>
  > => {
  ...
  const createNewCategory = async (
    toCreate: CategoryCreateSchema
  ): Promise<ApiResponse<CategoryReadSchema>> => {
  ...
  ): Promise<ApiResponse<ItemDetailReadSchema>> => {
  ...
  const consumeItemInstance = async (
    itemId: string,
    instanceId: string
  ...
  ): Promise<ApiResponse<ItemReadSchema>> => {
  ...
  const createItemInstance = async (
    item_id: string,
    item: ItemInstanceCreate
  ): Promise<ApiResponse<ItemReadSchema>> => {
  ...
  const updateItemInstance = async (
    itemId: string,
    instanceId: string,
    updateData: ItemInstanceUpdateSchema
  ): Promise<ApiResponse> => {
    const response = await api.patch(
      `/items/${itemId}/${instanceId}`,
      updateData
    )
    return {
      success: response.status == 204,
